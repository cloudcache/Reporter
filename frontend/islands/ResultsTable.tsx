...
export function ResultsTable({ rows }: ResultsTableProps) {
  ...
  if (error) return <FallbackTable rows={rows} error={error} />
  return <div ref={ref} className="h-[420px] w-full overflow-hidden rounded-lg border border-line bg-surface" />
}

function FallbackTable({ rows, error }: { rows: Row[]; error: string }) {
  const fields = Object.keys(rows[0] || {})
  return <div className="rounded-lg border border-line bg-surface">
    <div className="border-b border-line bg-amber-50 px-3 py-2 text-xs text-amber-700">VTable 降级渲染：{error}</div>
